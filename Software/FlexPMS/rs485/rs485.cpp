...
RS485::RS485(string port_path, int baud)
{
...
void RS485::sendChar(char ch, bool address) {
	...
	if(!write(tty_fd_, &ch, 1))
		cout << "I did not write anything\n";
}
...
void RS485::getPacket(char* packet)
{
	//unsigned int len;
	//char empty, 
	char in = 0;
	bool addr = false;
/*
	while(1 == getChar(packet[0]) && packet[0] != 1){};

//	cout << "R; " << (unsigned int)packet[0] << "\n";
	getChar(packet[1]);
//	cout << "T; " << (unsigned int)packet[1] << "\n";
	getChar(packet[2]);
//	cout << "L; " << (unsigned int)packet[2] << "\n";
	len = packet[2];
	getChar(packet[3]);
//	cout << "C; " << (unsigned int)packet[3] << "\n";
	for(unsigned int i = 4; i < len + 4; ++i){
		getChar(packet[i]);
//		cout << "D; " << (unsigned int)packet[i] << "\n";
	}
	...
	while(read(tty_fd_, &empty, 1) == 1){ cout << (unsigned int) empty << " ";}
	cout << endl;*/
...
void RS485::getMessege(char* msg)
{
